...
## Current Task: Phase 15 Batch A — Regime Detector + Statistical Gates
...
#### 1. Regime Decay Detector（Exp 1）
**檔案**：`src/tradememory/evolution/regime_detector.py`
**測試**：`tests/test_regime_detector.py`

三重確認系統（2/3 觸發才判定 decay）：
- **S1: Win Rate Decay** — Bayesian Beta updating。Prior = Beta(wins_backtest, losses_backtest)。Live data 更新 posterior。Trigger: P(win_rate < breakeven) > 0.80。用 `scipy.stats.beta`。
- **S2: Drawdown Exceedance** — current_DD > 1.5× backtest_MDD，且 ≥10 trades in drawdown period。
- **S3: Market OOD** — Mahalanobis distance on market features（ATR ratio, trend_12h_pct, atr_percentile）。用 `sklearn.covariance.EmpiricalCovariance`。Trigger: distance > 95th percentile of training distribution。

Anti-false-positive：
- min_trades = 20 before any decay assessment
- 2/3 signals 同時觸發才確認
- cooling_period = 5 trades after trigger（確認持續性）
- Bonferroni: alpha = 0.05/N（N = number of strategies monitored）

**Input**：
- 複用 `src/tradememory/evolution/models.py` — `FitnessMetrics`（backtest baseline stats）
- 複用 `src/tradememory/data/context_builder.py` — `MarketContext`（market features for OOD）

**Tests 要包含**：
- Synthetic stable scenario → no decay detected
- Synthetic decay scenario（win rate drops from 60% to 30%）→ decay detected
- Synthetic OOD scenario（ATR 突然 3x）→ OOD signal fires
- Edge case: < 20 trades → no assessment
- Cooling period behavior

#### 2. Statistical Gates（Exp 3）
**檔案**：`src/tradememory/evolution/statistical_gates.py`
**測試**：`tests/test_statistical_gates.py`

三個 pure functions：
- **`deflated_sharpe_ratio(observed_sr, num_trials, num_obs, skewness=0, kurtosis=3)`** → DSR value + p-value。Bailey & Lopez de Prado 2014 公式。
- **`min_backtest_length(target_sr, num_trials, alpha=0.05, skewness=0, kurtosis=3)`** → minimum number of observations needed。
- **`benjamini_hochberg(p_values, alpha=0.05)`** → list of (index, p_value, significant) tuples。可直接用 `statsmodels.stats.multitest.multipletests` 或手寫。

**Tests 要包含**：
- DSR with known values: M=1, SR=2.0, T=100 → DSR should be > 0（single trial, strong Sharpe）
- DSR with M=10000, SR=1.5, T=50 → DSR should be < 0（too many trials, short track record）
- MinBTL: M=1000, SR*=1.5 → should return reasonable number（~200-500）
- MinBTL: M=1, SR*=2.0 → should return small number（~30-50）
- BH-FDR: known p-values → known rejections

### 驗收標準
1. `python -m pytest tests/ -v` 全過（含新 tests，現有 1087 不能壞）
2. Synthetic decay scenario 跑過，detector 正確觸發
3. DSR(M=1000, SR=2.0, T=100) > 0 確認公式正確
4. 完成後 commit + 更新此 CLAUDE.md 的 Recent Changes
