#!/usr/bin/env python3
"""
TradeMemory Protocol — Rich Demo for Recording
================================================
Enhanced version of demo.py with rich formatting for terminal recording.
...
def main():
    ...
    table.add_column("Result", width=4)
    ...
    with Progress(
        ...
        task = progress.add_task("Analyzing 30 trades...", total=100)
        for _ in range(100):
            time.sleep(0.02)
    ...
    ptable.add_column("Assessment", width=12)
    ...
    for strategy in ["VolBreakout", "Pullback"]:
        ...
        ptable.add_row(
            ...
            f"[{pnl_style}]${st['pnl']:+.2f}[/{pnl_style}]",
            badge,
        )
    ...
    for session in ["london", "asian", "newyork"]:
        ...
        total = s["wins"] + s["losses"]
        wr = s["wins"] / total * 100 if total else 0
        patterns_found.append({"session": session, "wr": wr})

    for p in patterns_found:
        if p["wr"] < 50:
            adjustments.append({"param": f"{p['session']}_max_lot", "old": "0.05", "new": "0.025",
                                "reason": f"{p['session'].capitalize()} WR {p['wr']:.0f}% — reduce exposure"})
        elif p["wr"] >= 70:
            adjustments.append({"param": f"{p['session']}_max_lot", "old": "0.05", "new": "0.08",
                                "reason": f"{p['session'].capitalize()} WR {p['wr']:.0f}% — earned more room"})
    ...
    atable.add_column("Reason", width=40)
    ...
    # ── Step 4: Before/After ──
    console.print()
    console.rule("[bold cyan]Step 4: Before vs After[/bold cyan]")
    console.print()

    ba_table = Table(box=box.DOUBLE_EDGE, show_header=True, title_style="bold white")
    ba_table.add_column("Trade", width=10, style="bold")
    ba_table.add_column("Without TradeMemory", width=35, style="red")
    ba_table.add_column("With TradeMemory", width=35, style="green")

    ba_table.add_row("Trade 1", "AI analyzes market, gives recommendation", "Same")
    ba_table.add_row("Trade 5", "Starts fresh, no memory of past trades",
                     '"Past 4 Asian: 3 losses. Reducing lot 50%."')
    ba_table.add_row("Trade 15", "No idea what its win rate is",
                     '"London VolBreakout WR 73%. Full size."')
    ba_table.add_row("Trade 30", "Same mistakes repeated. No learning.",
                     "Auto-adjusted weights. Avoids weak sessions.")
